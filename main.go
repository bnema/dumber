...
func runBrowser() {
	...
	if err != nil {
		...
		view.RegisterScriptMessageHandler(func(payload string) {
			...
			case "theme":
				if m.Value != "" {
					log.Printf("[theme] page reported color-scheme: %s", m.Value)
				}
			}
		})
...
// readMemoryConfigFromEnv reads optional memory optimization knobs from environment variables.
// Defaults favor performance; only applied when env vars are set.
func readMemoryConfigFromEnv() webkit.MemoryConfig {
	var mc webkit.MemoryConfig

	// Cache model (document_viewer | web_browser | primary_web_browser)
	if v := os.Getenv("DUMBER_CACHE_MODEL"); v != "" {
	...
	// Page cache toggle (default true, only override if provided)
	if v := os.Getenv("DUMBER_ENABLE_PAGE_CACHE"); v != "" {
		mc.EnablePageCache = v == "1" || strings.EqualFold(v, "true") || strings.EqualFold(v, "yes")
	} else {
		mc.EnablePageCache = true
	}

	// Memory limit (MB)
	if v := os.Getenv("DUMBER_MEMORY_LIMIT_MB"); v != "" {
	...
	// Thresholds
	if v := os.Getenv("DUMBER_MEMORY_CONSERVATIVE"); v != "" {
		if f, err := strconv.ParseFloat(v, 64); err == nil {
			mc.ConservativeThreshold = f
		}
	}
	if v := os.Getenv("DUMBER_MEMORY_STRICT"); v != "" {
		if f, err := strconv.ParseFloat(v, 64); err == nil {
			mc.StrictThreshold = f
		}
	}
	if v := os.Getenv("DUMBER_MEMORY_KILL"); v != "" {
		if f, err := strconv.ParseFloat(v, 64); err == nil {
			mc.KillThreshold = f
		}
	}
	if v := os.Getenv("DUMBER_MEMORY_POLL_INTERVAL"); v != "" {
		if f, err := strconv.ParseFloat(v, 64); err == nil {
			mc.PollIntervalSeconds = f
		}
	}

	// Periodic GC (seconds)
	if v := os.Getenv("DUMBER_GC_INTERVAL"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n >= 0 {
			mc.EnableGCInterval = n
		}
	}

	// Recycle after N page loads
	if v := os.Getenv("DUMBER_RECYCLE_THRESHOLD"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n >= 0 {
			mc.ProcessRecycleThreshold = n
		}
	}

	// Enable memory monitoring logs
	if v := os.Getenv("DUMBER_ENABLE_MEMORY_MONITORING"); v != "" {
		mc.EnableMemoryMonitoring = v == "1" || strings.EqualFold(v, "true") || strings.EqualFold(v, "yes")
	}
