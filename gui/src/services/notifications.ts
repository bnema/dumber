...
export class NotificationService {
  ...
  showZoom(message: string, duration = 2000): void {
    ...
    if (!this.zoomNotification) {
      this.zoomNotification = this.createNotificationElement(
        "zoom-notification",
        {
          top: "20px",
          right: "20px",
  ...
  show(
    ...
    if (!this.generalNotification) {
      this.generalNotification = this.createNotificationElement(
  ...
  private getNotificationColors(type: NotificationOptions["type"]): {
    background: string;
    color: string;
  } {
    switch (type) {
