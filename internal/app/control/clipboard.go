...
func (c *ClipboardController) CopyCurrentURL() {
	...
	if currentURL == "" {
		...
		_ = c.webView.InjectScript(`window.__dumber_showToast && window.__dumber_showToast("No URL to copy", 2000)`)
		return
	}

	if err := clipboard.CopyToClipboard(currentURL); err != nil {
		log.Printf("Failed to copy URL to clipboard: %v", err)
		_ = c.webView.InjectScript(`window.__dumber_showToast && window.__dumber_showToast("Failed to copy URL", 2000)`)
	} else {
		log.Printf("URL copied to clipboard: %s", currentURL)
		_ = c.webView.InjectScript(`window.__dumber_showToast && window.__dumber_showToast("URL copied to clipboard", 2000)`)
	}
}
