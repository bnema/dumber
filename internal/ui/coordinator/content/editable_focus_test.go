...
func TestEditableFocusCallback_ForwardsPaneID(t *testing.T) {
	...
	wv.EXPECT().Generation().Return(uint64(0)).Twice()
...
func TestEditableFocusCallback_BackgroundPaneDoesNotConfusePaneOwnership(t *testing.T) {
	...
	wvA.EXPECT().ID().Return(port.WebViewID(1)).Once()
	wvA.EXPECT().Generation().Return(uint64(0)).Twice()
	wvA.EXPECT().SetCallbacks(mock.Anything).Run(func(cb *port.WebViewCallbacks) {
		callbacksA = cb
	}).Once()
	wvB.EXPECT().ID().Return(port.WebViewID(2)).Once()
	wvB.EXPECT().Generation().Return(uint64(0)).Twice()
