...
func (*SchemaProvider) getWorkspaceKeys(defaults *Config) []entity.ConfigKeyInfo {
	return []entity.ConfigKeyInfo{
		...
		// Browsing contexts
		{
			...
			Type:        "string",
			Default:     string(defaults.Workspace.BrowsingContexts.Behavior),
			Description: "How to handle JavaScript popup windows",
			Values:      []string{"split", "stacked", "tabbed", "windowed"},
			Section:     SectionWorkspace,
		},
		{
			Key:         "workspace.browsing_contexts.placement",
			Type:        "string",
			Default:     defaults.Workspace.BrowsingContexts.Placement,
			Description: "Placement direction for split popups",
			Values:      []string{"right", "left", "top", "bottom"},
			Section:     SectionWorkspace,
		},
		{
			Key:         "workspace.browsing_contexts.blank_target_behavior",
			Type:        "string",
			Default:     defaults.Workspace.BrowsingContexts.BlankTargetBehavior,
			Description: "How to handle target=\"_blank\" links",
			Values:      []string{"split", "stacked", "tabbed"},
			Section:     SectionWorkspace,
		},
		{
			Key:         "workspace.browsing_contexts.open_in_new_pane",
			Type:        "bool",
			Default:     fmt.Sprintf("%t", defaults.Workspace.BrowsingContexts.OpenInNewPane),
			Description: "Open popups in workspace instead of blocking",
			Section:     SectionWorkspace,
		},
		{
			Key:         "workspace.browsing_contexts.follow_pane_context",
			Type:        "bool",
			Default:     fmt.Sprintf("%t", defaults.Workspace.BrowsingContexts.FollowPaneContext),
			Description: "Popup placement follows parent pane context",
			Section:     SectionWorkspace,
		},
		{
			Key:         "workspace.browsing_contexts.enable_smart_detection",
			Type:        "bool",
			Default:     fmt.Sprintf("%t", defaults.Workspace.BrowsingContexts.EnableSmartDetection),
			Description: "Use window properties to detect popup intent",
			Section:     SectionWorkspace,
		},
		{
			Key:         "workspace.browsing_contexts.oauth_auto_close",
			Type:        "bool",
			Default:     fmt.Sprintf("%t", defaults.Workspace.BrowsingContexts.OAuthAutoClose),
		...
		},
		{
			...
			Description: "Reserved; mode frame borders currently use fixed widths",
			Range:       ">=0",
			Section:     SectionWorkspace,
		},
		{
		...
		{
			Key:         "workspace.styling.mode_legend_delay_ms",
			Type:        "int",
		...
		{
			Key:         "workspace.styling.mode_legend_animations",
			Type:        "bool",
			Default:     fmt.Sprintf("%t", defaults.Workspace.Styling.ModeLegendAnimations),
			Description: "Animate the mode legend and pressed keycaps",
			Section:     SectionWorkspace,
		},
		{
...
func (*SchemaProvider) getPerformanceKeys(defaults *Config) []entity.ConfigKeyInfo {
	return []entity.ConfigKeyInfo{
		...
		{
			Key:         "engine.cef.adaptive_windowless_frame_rate",
			Type:        "bool",
			Default:     fmt.Sprintf("%t", defaults.Engine.CEF.AdaptiveWindowlessFrameRate),
			Description: "Adapt CEF OSR frame rate to the active Wayland monitor refresh rate when no explicit windowless_frame_rate is set",
			Section:     SectionPerformance,
		},
		{
			Key:         "engine.cef.windowless_frame_rate",
			Type:        "int32",
			Default:     fmt.Sprintf("%d", defaults.Engine.CEF.WindowlessFrameRate),
			Description: "Explicit maximum off-screen frame rate for the CEF engine; 0 enables adaptive mode when configured",
			Range:       ">=0",
			Section:     SectionPerformance,
		},
		{
		...
		},
		{
			...
			Description: "CEF mouse wheel scroll delta multiplier",
			Range:       ">0",
			Section:     SectionPerformance,
		},
		{
			...
			Description: "CEF precise/surface scroll delta multiplier for touchpads and high-resolution wheels",
			Range:       ">0",
			Section:     SectionPerformance,
		},
		{
			Key:         "engine.cef.input.scroll_horizontal_multiplier",
			Type:        "float64",
			Default:     fmt.Sprintf("%.2f", defaults.Engine.CEF.Input.ScrollHorizontalMultiplier),
			Description: "CEF horizontal scroll delta multiplier",
			Range:       ">0",
			Section:     SectionPerformance,
		},
		{
			Key:         "engine.cef.input.scroll_vertical_multiplier",
			Type:        "float64",
			Default:     fmt.Sprintf("%.2f", defaults.Engine.CEF.Input.ScrollVerticalMultiplier),
			Description: "CEF vertical scroll delta multiplier; touchpad vertical speed uses this together with scroll_precise_multiplier",
			Range:       ">0",
			Section:     SectionPerformance,
		},
		{
			Key:         "engine.cef.input.scroll_max_delta",
			Type:        "int32",
			Default:     fmt.Sprintf("%d", defaults.Engine.CEF.Input.ScrollMaxDelta),
			Description: "CEF maximum absolute scroll delta after scaling; 0 disables clamping",
			Range:       ">=0",
			Section:     SectionPerformance,
		},
		{
		...
		},
		{
			...
			Description: "Enable deliberate two-finger touchpad swipe back/forward navigation for CEF views",
			Section:     SectionPerformance,
		},
		{
			Key:         "engine.cef.input.touchpad_navigation_min_delta",
			Type:        "float64",
			Default:     fmt.Sprintf("%.2f", defaults.Engine.CEF.Input.TouchpadNavigationMinDelta),
			Description: "Minimum accumulated horizontal touchpad swipe delta required before release triggers CEF back/forward navigation",
			Range:       ">0",
			Section:     SectionPerformance,
		},
		{
