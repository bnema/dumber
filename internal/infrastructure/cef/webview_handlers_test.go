...
func TestOnLoadStartFiresCommittedAndUpdatesURI(t *testing.T) {
	...
	if len(gotEvents) != 1 {
		t.Fatalf("expected 1 load event, got %d", len(gotEvents))
	}
	if gotEvents[0] != port.LoadCommitted {
		t.Fatalf("expected %v, got %v", port.LoadCommitted, gotEvents[0])
	}
	if gotURI := wv.URI(); gotURI != "https://google.com" {
		t.Fatalf("expected URI to update from frame on load commit, got %q", gotURI)
	}
}
