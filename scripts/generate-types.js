...
const TS_OUTPUT_PATH = 'gui/src/types/generated.ts';
...
function parseGoStruct(goCode, structName) {
  ...
  const fields = mergedFields.map(fieldLine => {
    ...
    const [, , goType, jsonTag] = fieldMatch;
...
function findGoEnums(goCode) {
  ...
  if (goCode.includes('type InputType int')) {
    ...
    const constRegex = /const\s*\(\s*([\s\S]*?)\s*\)/;
...
`;
  ...
  for (const { path, files } of sourceFiles) {
    for (const filename of files) {
      ...
      for (const enumDef of enumsInFile) {
        if (generatedEnums.has(enumDef.name)) {
          continue;
        }
        generatedEnums.add(enumDef.name);
  ...
  combinedTypes += `// Manual types and aliases
${generatedEnums.has('InputType') ? '' : `export enum InputType {
