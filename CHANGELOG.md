...
- **Named browser instances**: Added `dumber browse --instance <name>` for deterministic, independent CEF/IPC namespaces while sharing configuration, history, favorites, and other user data.
...
- **Omnibox borders**: Outer borders scale with the configured UI size and remain more pronounced for the default `command` style.
...
- **AUR git package CI**: Automated CI workflow to publish `dumber-browser-git` AUR package on push to main branch.
